...
// Ask the Wayback Machine to preserve the page, and return the best snapshot URL we can.
// Best-effort: a fact-checked page often gets edited/deleted, so we keep a permanent copy.
async function archiveSnapshot(url) {
  // Trigger a fresh capture (fire-and-forget; Save Page Now can be slow).
  try {
    const c = new AbortController();
    setTimeout(() => c.abort(), 1500);
    fetch('https://web.archive.org/save/' + url, { method: 'GET', signal: c.signal }).catch(() => {});
  } catch (e) {}
  // Return the closest existing snapshot if one is available.
  try {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 8000);
    const r = await fetch('https://archive.org/wayback/available?url=' + encodeURIComponent(url), { signal: ctrl.signal });
    clearTimeout(t);
...
module.exports = { scrape, normalizeUrl, domainOf, detectSourceType, archiveSnapshot };
