...
async function analyse({ policyBody, title, text, url }) {
  ...
  const body = {
    model: process.env.KN_MODEL || 'claude-opus-4-8',
    max_tokens: 3000,
  ...
  return {
    ...
    suggested_category: CATEGORIES.includes(parsed.suggested_category) ? parsed.suggested_category : 'unverified',
